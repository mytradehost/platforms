{File: _ContinuousAdoptRealWorldPositionV3.el}

{  _ContinuousAdoptRealWorldPositionV3:    
  
   Last Updated: 7/30/2014  
     
   Same as V2, but:  
     
   1) Replaces MACRO ".CAOSA" with OrderTicket CancelAllOrders method. 
   2) Added input to select whether all orders or only DAY orders are canceled 
   3) Replaced LastBarOnChart with LastBarOnChartEx  
...
inputs:     
	string AccountID( "" ),				//  if AccountID is not entered, it uses the default provided by GetAccountID.     
	  
	int PosMisMatchTimeOutMS( 15000 ),	//  Milliseconds since strategy vs actual position mismatch discovered   
	...
	bool CancelAllOrders( false ),	    //  if true, then when a position mismatch is detected, all open orders for  
	...
	bool CancelDayOrdersOnly( false ),	// if true and using CancelAllOrders then only orders with duration "DAY" will 
									// be canceled. All strategy orders are day orders. This allows you leave existing 
									// orders such as "GTC" in place. Default is to cancel all open orders for the account 
									// and symbol. 
	                                  
	bool PrintDebug( false ) ;	        //  if true, prints out debug data to the Print Log    
...
Method void PosMisMatchTimeOut( elsystem.Object sender, elsystem.TimerElapsedEventArgs args ) begin   
	...
	if CancelAllOrders then  
		OrderTicket.CancelAllOrders( MyAccountID, Symbol, CancelDayOrdersOnly ) ;  
...
Once begin     
	...
	//  Verify that OT's are enabled for the strategy.    
	If not OrderTicket.OrderPlacementEnabled then 
		RaiseRunTimeError( "Must enable Order Placement Objects on General Tab of the Strategy Properties Window" ) ;  
...
if LastBarOnChartEx and RealTimeTicks = false and GetAppInfo( aiRealTimeCalc ) = 1 then  
	RealTimeTicks = true ;  
