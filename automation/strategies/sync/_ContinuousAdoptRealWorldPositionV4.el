{File: _ContinuousAdoptRealWorldPositionV4.el}

{  _ContinuousAdoptRealWorldPositionV4   
  
   Last Updated: 8/15/2013 Afternoon  
     
   *** Reqires V9.1 Update 13 or later. ***  
     
   *** Same as earlier versions, but adds the ability to have different time outs for   
   converting entry and exit Limit orders to Market orders so many seconds after   
   the Limit price is touched.  ***  
...
[IntrabarOrderGeneration = true]  // We must process each tick when we have no position but  
                                  //  limit orders whose price may have been touched, but have  
                                  //  not filled.  
...
Inputs:  
	_Note_1 ("Must Have Adopt Real World Checked"), 
	_Note_2 ("Must Have IOG On"), 
	_Note_3 ("Must Have Run Command On?No Confirmations"), 
	_Note_4 ("Must Have Objects Turned ON"),    
	...
	For Cnt = 0 to OrdCount - 1 Begin  
		...
		If  MyOrder.Type = OrderType.Limit and   
		  ( MyOrder.Action = OrderAction.buy or MyOrder.Action = OrderAction.sellshort ) and  
			...
			If PrintDebug Then  
				Print( "Issuing Market Order to immediately fill ENTRY Limit order at Time = ", DateTimeToString(ComputerDateTime) ) ;  
	...
	For Cnt = 0 to OrdCount - 1 Begin  
		...
		If  MyOrder.Type = OrderType.Limit and   
		  ( MyOrder.Action = OrderAction.sell or MyOrder.Action = OrderAction.buytocover ) and  
			...
			If PrintDebug Then  
				Print( "Issuing Market Order to immediately fill EXIT Limit order at Time = ", DateTimeToString(ComputerDateTime) ) ;  
...
Once Begin     
	...
	SHost         = new StrategyHost;  
	SHost.Connect = True;   
	  
	If PrintDebug Then  
		Print( " Strat Adopt Real-World Pos=", SHost.Automation.AdoptRealWorldPosition.ToString(),   
		       " and Automated Trading with Confirmation=", SHost.Automation.Mode.ToString() ) ;  
	  
	//  Verify the strategy can adopt the real-world position and has automation fully enabled.  
	If SHost.Automation.AdoptRealWorldPosition = False or   
	   SHost.Automation.Mode <> AutomationMode.fullnoconfirm Then  
	        RaiseRunTimeError( "Enable Full Automation & Adopt Real-World Position on Automation Tab" ) ;  
	  
	SHost = NULL ;  //  Clear the strategy host object as we no longer require it  
	...
	If PrintDebug Then   
		print( Newline, " *** CONTINUOUS ADOPT THE REAL-WORLD POSITION V4 STARTING ***", NewLine ) ;   
...
//  Create code which references exit orders for both long and short, but which cannot execute   
//  so this strategy will not be disabled upon restart.  
Once Begin  
